...
public class EnumRecordGenerator : IIncrementalGenerator
{
    ...
    private const string RequireEnumRecordAttributeName = "RequireEnumRecordAttribute";
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        context.RegisterSourceOutput(collectedEnums, static (ctx, enumInfos) =>
        {
            ...
            {
                // Generate IHas{PropertyName} marker interfaces
                var interfacesSource = GeneratePropertyInterfaces(validEnums);
                ctx.AddSource("EnumRecordInterfaces.g.cs", SourceText.From(interfacesSource, Encoding.UTF8));

                // Generate the central EnumRecord lookup class with Resolve<T>
    ...
    private static string GeneratePropertyInterfaces(List<EnumInfo> enumInfos)
    {
        var sb = new StringBuilder();
        ...
        sb.AppendLine("#nullable enable");
        sb.AppendLine();
        sb.AppendLine("namespace EnumRecords.Contracts;");
        sb.AppendLine();
        sb.AppendLine("/// <summary>");
        sb.AppendLine("/// Marker interfaces for EnumRecord property discrimination.");
        sb.AppendLine("/// These interfaces are automatically generated based on property names");
        sb.AppendLine("/// found in EnumRecord property structs across the assembly.");
        sb.AppendLine("/// </summary>");

        // Collect all unique property names with their types
        var propertySignatures = new Dictionary<string, HashSet<string>>();
        foreach (var enumInfo in enumInfos)
        {
            foreach (var property in enumInfo.Properties)
            {
                if (!propertySignatures.TryGetValue(property.Name, out var types))
                {
                    types = new HashSet<string>();
                    propertySignatures[property.Name] = types;
                }
                types.Add(property.TypeName);
            }
        }

        // Generate marker interface for each unique property name
        foreach (var kvp in propertySignatures.OrderBy(p => p.Key))
        {
            var propertyName = kvp.Key;
            sb.AppendLine();
            sb.AppendLine($"/// <summary>");
            sb.AppendLine($"/// Marker interface indicating an EnumRecord properties struct has a '{propertyName}' property.");
            sb.AppendLine($"/// </summary>");
            sb.AppendLine($"public interface IHas{propertyName} {{ }}");
        }
    ...
    private static string GenerateEnumRecordLookup(List<EnumInfo> enumInfos)
    {
        ...
        // Generate accessor interface and struct for generic resolution
        sb.AppendLine("/// <summary>");
        sb.AppendLine("/// Interface for accessing EnumRecord properties generically.");
        sb.AppendLine("/// </summary>");
        sb.AppendLine("/// <typeparam name=\"TEnum\">The enum type.</typeparam>");
        sb.AppendLine("public interface IEnumRecordAccessor<TEnum> where TEnum : struct, global::System.Enum");
        sb.AppendLine("{");
        sb.AppendLine("    /// <summary>Gets the value of a named property for the given enum value.</summary>");
        sb.AppendLine("    object? GetPropertyValue(TEnum value, string propertyName);");
        sb.AppendLine("    /// <summary>Gets all property names available on this EnumRecord.</summary>");
        sb.AppendLine("    global::System.Collections.Generic.IReadOnlyList<string> PropertyNames { get; }");
        sb.AppendLine("}");
        sb.AppendLine();

        // Generate accessor structs for each enum
        foreach (var enumInfo in enumInfos)
        {
            var fullEnumTypeName = enumInfo.Namespace != null
            ...
            sb.AppendLine($"/// <summary>");
            sb.AppendLine($"/// Accessor struct for <see cref=\"{enumInfo.EnumName}\"/> EnumRecord.");
            sb.AppendLine($"/// </summary>");
            sb.AppendLine($"public readonly struct {enumInfo.EnumName}Accessor : IEnumRecordAccessor<{fullEnumTypeName}>");
            sb.AppendLine("{");
            
            // Property names array
            var propNames = string.Join(", ", enumInfo.Properties.Select(p => $"\"{p.Name}\""));
            sb.AppendLine($"    private static readonly string[] _propertyNames = new[] {{ {propNames} }};");
            sb.AppendLine();
            sb.AppendLine($"    /// <inheritdoc />");
            sb.AppendLine($"    public global::System.Collections.Generic.IReadOnlyList<string> PropertyNames => _propertyNames;");
            sb.AppendLine();

            // GetPropertyValue implementation
            sb.AppendLine($"    /// <inheritdoc />");
            sb.AppendLine($"    public object? GetPropertyValue({fullEnumTypeName} value, string propertyName) => propertyName switch");
            sb.AppendLine("    {");
            foreach (var property in enumInfo.Properties)
            {
                sb.AppendLine($"        \"{property.Name}\" => {fullRecordTypeName}.Get{property.Name}(value),");
            }
            sb.AppendLine($"        _ => throw new global::System.ArgumentException($\"Property '{{propertyName}}' not found on {enumInfo.EnumName}\", nameof(propertyName))");
            sb.AppendLine("    };");

            // Typed property accessor methods
            foreach (var property in enumInfo.Properties)
            {
                sb.AppendLine();
                sb.AppendLine($"    /// <summary>Gets the {property.Name} value for the specified enum value.</summary>");
                sb.AppendLine($"    public {property.TypeName} Get{property.Name}({fullEnumTypeName} value) => {fullRecordTypeName}.Get{property.Name}(value);");
            }
        ...
        // Generate the Resolve<T>() method using switch pattern matching
        sb.AppendLine("    /// <summary>");
        sb.AppendLine("    /// Resolves the EnumRecord accessor for the specified enum type.");
        sb.AppendLine("    /// </summary>");
        sb.AppendLine("    /// <typeparam name=\"TEnum\">The enum type with an associated EnumRecord.</typeparam>");
        sb.AppendLine("    /// <returns>An accessor struct for the enum's properties.</returns>");
        sb.AppendLine("    /// <exception cref=\"global::System.InvalidOperationException\">Thrown if the enum type does not have an associated EnumRecord.</exception>");
        sb.AppendLine("    public static IEnumRecordAccessor<TEnum> Resolve<TEnum>() where TEnum : struct, global::System.Enum");
        sb.AppendLine("    {");
        sb.AppendLine("        return typeof(TEnum) switch");
        sb.AppendLine("        {");
        ...
        foreach (var enumInfo in enumInfos)
        {
            var fullEnumTypeName = enumInfo.Namespace != null
                ? $"global::{enumInfo.Namespace}.{enumInfo.EnumName}"
                : $"global::{enumInfo.EnumName}";
            sb.AppendLine($"            {{ }} type when type == typeof({fullEnumTypeName}) => (IEnumRecordAccessor<TEnum>)(object)new {enumInfo.EnumName}Accessor(),");
        }
        
        sb.AppendLine("            _ => throw new global::System.InvalidOperationException($\"No EnumRecord found for type {typeof(TEnum).FullName}\")");
        sb.AppendLine("        };");
        sb.AppendLine("    }");
        sb.AppendLine();

        // Generate TryResolve<T>() method using switch pattern matching
        sb.AppendLine("    /// <summary>");
        sb.AppendLine("    /// Attempts to resolve the EnumRecord accessor for the specified enum type.");
        sb.AppendLine("    /// </summary>");
        sb.AppendLine("    /// <typeparam name=\"TEnum\">The enum type with an associated EnumRecord.</typeparam>");
        sb.AppendLine("    /// <param name=\"accessor\">When successful, contains the accessor struct.</param>");
        sb.AppendLine("    /// <returns>True if an EnumRecord exists for the type; otherwise, false.</returns>");
        sb.AppendLine("    public static bool TryResolve<TEnum>([global::System.Diagnostics.CodeAnalysis.NotNullWhen(true)] out IEnumRecordAccessor<TEnum>? accessor) where TEnum : struct, global::System.Enum");
        sb.AppendLine("    {");
        sb.AppendLine("        (accessor, var success) = typeof(TEnum) switch");
        sb.AppendLine("        {");
        ...
        foreach (var enumInfo in enumInfos)
        {
            var fullEnumTypeName = enumInfo.Namespace != null
                ? $"global::{enumInfo.Namespace}.{enumInfo.EnumName}"
                : $"global::{enumInfo.EnumName}";
            sb.AppendLine($"            {{ }} type when type == typeof({fullEnumTypeName}) => ((IEnumRecordAccessor<TEnum>?)(object)new {enumInfo.EnumName}Accessor(), true),");
        }
...
/// <summary>
/// Specifies that a generic method or class requires the type parameter to have an
/// EnumRecord whose properties type implements the specified interface.
/// This constraint is enforced by the EnumRecords analyzer at compile time.
/// </summary>
/// <typeparam name=""TInterface"">The interface that the EnumRecord properties struct must implement (e.g., IHasHexCode).</typeparam>
/// <remarks>
/// <para>
/// When applied to a method, any call site with a concrete enum type will be validated
/// to ensure the enum has an [EnumRecord&lt;T&gt;] attribute where T implements TInterface.
/// </para>
/// <para>
/// When a generic method calls another method with this attribute, the calling method
/// must also have the same (or stricter) RequireEnumRecord constraint for the constraint
...
/// public static string GetHex&lt;TEnum&gt;(TEnum value) where TEnum : struct, Enum
///     =&gt; EnumRecord.Resolve&lt;TEnum&gt;().GetPropertyValue(value, ""HexCode"")?.ToString() ?? """";
/// </code>
/// </example>
[global::System.AttributeUsage(global::System.AttributeTargets.Method | global::System.AttributeTargets.Class, AllowMultiple = true, Inherited = true)]
public sealed class RequireEnumRecordAttribute<TInterface> : global::System.Attribute
{
    /// <summary>
    /// Gets or sets the index of the type parameter this constraint applies to.
    /// Defaults to 0 (the first type parameter).
    /// </summary>
    public int TypeParameterIndex { get; set; } = 0;
}
";
